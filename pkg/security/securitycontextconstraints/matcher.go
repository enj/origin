...
func (d *defaultSCCMatcher) FindApplicableSCCs(userInfo user.Info, namespace string) ([]*securityapi.SecurityContextConstraints, error) {
	...
	for _, constraint := range constraints {
		if ConstraintAppliesTo(constraint, userInfo) || d.authorizedForSCC(constraint, userInfo, namespace) {
...
func (d *defaultSCCMatcher) authorizedForSCC(constraint *securityapi.SecurityContextConstraints, info user.Info, namespace string) bool {
	...
	decision, reason, err := d.authorizer.Authorize(attr)
	if err != nil {
