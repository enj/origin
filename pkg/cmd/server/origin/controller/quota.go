...
func RunResourceQuotaManager(ctx ControllerContext) (bool, error) {
	...
	resourceQuotaRegistry := quota.NewAllResourceQuotaRegistry(
		ctx.DeprecatedOpenshiftInformers,
		ctx.ImageInformers.Image().InternalVersion().ImageStreams(),
		ctx.ClientBuilder.DeprecatedOpenshiftClientOrDie(bootstrappolicy.InfraResourceQuotaControllerServiceAccountName),
		ctx.ClientBuilder.ClientOrDie(bootstrappolicy.InfraResourceQuotaControllerServiceAccountName),
	)

	resourceQuotaControllerOptions := &kresourcequota.ResourceQuotaControllerOptions{
		KubeClient:            ctx.ClientBuilder.ClientOrDie(bootstrappolicy.InfraResourceQuotaControllerServiceAccountName),
		ResourceQuotaInformer: ctx.DeprecatedOpenshiftInformers.KubernetesInformers().Core().V1().ResourceQuotas(),
...
func (c *ClusterQuotaReconciliationControllerConfig) RunController(ctx ControllerContext) (bool, error) {
	...
	options := clusterquotareconciliation.ClusterQuotaReconcilationControllerOptions{
		ClusterQuotaInformer: ctx.DeprecatedOpenshiftInformers.ClusterResourceQuotas(),
